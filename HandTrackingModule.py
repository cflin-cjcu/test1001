...
class HandDetector:
    ...
    def findHands(self, img, draw=True):
        ...
        drawspec_point = self.mpDraw.DrawingSpec(
            thickness=2, circle_radius=1, color=(255, 255, 0))
        drawspec_line = self.mpDraw.DrawingSpec(
            thickness=2, color=(0, 255, 255))
        if self.results.multi_hand_landmarks:
            for handLms in self.results.multi_hand_landmarks:
                if draw:
                    self.mpDraw.draw_landmarks(img, handLms, self.mpHands.HAND_CONNECTIONS,
                                               drawspec_point, drawspec_line)
        return img
